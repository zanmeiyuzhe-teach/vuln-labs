...
('admin', 'admin123', 'admin@cyberrange.local', 'admin', 10000.00),
('victim', 'victim123', 'victim@cyberrange.local', 'user', 1000.00),
('attacker', 'attacker123', 'attacker@cyberrange.local', 'user', 100.00);

-- Seed flags
INSERT INTO flags (flag, description) VALUES
('CR{csrf_get_easy}', 'CSRF Easy - GET-based CSRF flag'),
('CR{csrf_post_simple}', 'CSRF Simple - POST-based CSRF flag'),
('CR{csrf_token_bypass}', 'CSRF Hard - Token bypass flag'),
('CR{csrf_xss_combo}', 'CSRF Hell - CSRF+XSS combo flag');
...
echo "[*] CSRF Lab database initialized"
    ...
    <div class="container">
        ...
        <div class="cards">
            <a href="/easy/" class="card">
                ...
                <h3 style="margin-top: 0.5rem;">GET 型 CSRF</h3>
                <p>转账功能使用 GET 请求，可直接通过 img 标签触发</p>
            </a>
            <a href="/simple/" class="card">
                <span class="badge simple">Simple</span>
                <h3 style="margin-top: 0.5rem;">POST 型 CSRF</h3>
                <p>转账功能使用 POST 请求，需要构造表单自动提交</p>
            </a>
            <a href="/hard/" class="card">
                <span class="badge hard">Hard</span>
                <h3 style="margin-top: 0.5rem;">Token 绕过</h3>
                <p>有 CSRF Token 保护，但存在漏洞可以绕过</p>
            </a>
            <a href="/hell/" class="card">
                <span class="badge hell">Hell</span>
                <h3 style="margin-top: 0.5rem;">CSRF + XSS 组合</h3>
                <p>需要结合 XSS 漏洞实现 CSRF 攻击</p>
            </a>
        </div>
    </div>
</body>
