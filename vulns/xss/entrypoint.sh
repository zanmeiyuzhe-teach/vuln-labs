...
# Start MySQL for stored XSS lab
service mariadb start
sleep 2

# Create database and seed data for stored XSS
mysql -u root <<'SQL'
...
-- Seed comments for stored XSS lab
INSERT INTO comments (username, content) VALUES
('admin', '欢迎来到留言板！请注意网络安全。'),
('user1', '这个网站看起来不错！'),
('user2', '我正在学习 XSS 漏洞。');

-- Seed flags for each difficulty
INSERT INTO flags (flag, description) VALUES
('CR{xss_reflected_easy}', 'XSS Easy - Reflected XSS via GET parameter'),
('CR{xss_stored_comment}', 'XSS Simple - Stored XSS via comment'),
('CR{xss_dom_based}', 'XSS Hard - DOM-based XSS'),
('CR{xss_csp_bypass}', 'XSS Hell - CSP bypass');
...
echo "[*] XSS Lab database initialized"
...
<head>
    <meta charset="UTF-8">
    <title>XSS Cross-Site Scripting Lab</title>
    <style>
    ...
    <div class="container">
        ...
        <div class="cards">
            <a href="/easy/" class="card">
                ...
                <h3 style="margin-top: 0.5rem;">反射型 XSS</h3>
                <p>GET 参数直接输出到页面，无任何过滤</p>
            </a>
            <a href="/simple/" class="card">
                <span class="badge simple">Simple</span>
                <h3 style="margin-top: 0.5rem;">存储型 XSS</h3>
                <p>留言板功能，输入存储后显示</p>
            </a>
            <a href="/hard/" class="card">
                <span class="badge hard">Hard</span>
                <h3 style="margin-top: 0.5rem;">DOM 型 XSS</h3>
                <p>JavaScript 处理 URL 片段导致漏洞</p>
            </a>
            <a href="/hell/" class="card">
                <span class="badge hell">Hell</span>
                <h3 style="margin-top: 0.5rem;">CSP 绕过</h3>
                <p>有 CSP 保护，需要绕过策略执行 XSS</p>
            </a>
        </div>
    </div>
</body>
